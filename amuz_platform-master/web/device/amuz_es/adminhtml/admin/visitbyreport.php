                ...
                <div class="row">
                    <div class="col-lg-12">
						<div class="panel panel-primary">
                                                        <?php if(!isset($_GET["dsk"])) { ?>
							<div class="panel-heading">Report Summary for <?=ucfirst($_GET["val"]);?>s</div>
							<?php } else { ?>
							<div class="panel-heading">Report Summary for <?=ucfirst($_GET["val"]);?>s on device: <?=ucfirst($_GET["dsk"])?> </div>
							<?php } ?>	
							...
							<table id="examplea" class='table table-bordered display'>
								...
								<tbody>
									...
									global $dblink;
										$sql = mysql_query("select custom_var_v4,custom_var_v6,custom_var_v7 from piwik_log_link_visit_action where custom_var_v4 = '".$_GET["val"]."' and custom_var_v1 like '%".$_GET["dsk"]."%'");
										$tnum = array();
