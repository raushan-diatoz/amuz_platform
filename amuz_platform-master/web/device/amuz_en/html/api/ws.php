...
//for devices
function validatecouponcode1($couponcode, $media){
  		...
  		$ping = "www.google.com";
             ...
             curl_close($ch1);  
              if($httpcode>=200 && $httpcode<400){
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, webserviceurl."/api/ws.php?action=cv&couponcode=".$couponcode."&medianame=".$media);
               	curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
                curl_setopt($ch, CURLOPT_HEADER, FALSE);
                echo $response = curl_exec($ch);
                curl_close($ch);
		...
		echo '{"response_code: 0, "response: internet not available" }';
	     }	
...
function getChannels($dir)
{
	...
	foreach($paths as $subfolder)
	{
		...
		{
			$vals[$val['name']] = 'http://'.ipaddress.'/api/ws.php?action=mf&dir='.$val['path'];
		...
		foreach(scandir($dir) as $f) {
			...
			if(in_array($path_parts['extension'], $imageextn)){
				...
				if(array_key_exists("extension", $path_parts)){
					...
					"title" => $meta[2],
					"desc" => $meta[3],
					"adpath" =>  str_replace(' ','%20','http://'.ipaddress.ADPATH.'/'.$meta[4] ),
					"rating" => $meta[5],
					"thumbnail" =>  str_replace(' ','%20','http://'.ipaddress.$dir.'/'.$path_parts['filename'].'.jpg'),
					"is_premium" => $meta[6],
					"roll" => $meta[7],
					"published"=>$meta[9],
					"price" => $meta[10],
					"premiumtime" => $meta[11],
					"skip_flag" => $meta[8],
					 "link" => str_replace(' ','%20','http://'.ipaddress.$dir . '/' . $f)
